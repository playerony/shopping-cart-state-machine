  ...
  req
    ...
    .on('end', () => {
      try {
        ...
        return resp
          .status(406)
          .json(newResponse(true, false, 'Incorrect input data', 'INCORRECT_BODY'));
      }
      ...
      if (!body.pid || !body.quantity) {
        return resp
          .status(406)
          .json(newResponse(true, false, 'Missing property', 'MISSING_PROPERTY'));
      }

      const product = cart.find((_product) => _product.pid === body.pid);
      ...
      if (body.quantity < product.min || body.quantity > product.max) {
        return resp
          .status(406)
          .json(newResponse(true, false, 'Incorrect product quantity', 'INCORRECT_QUANTITY'));
      }

      return resp.status(200).json(newResponse(false, true, 'Correct product', ''));
    });
});
